...
export default function useFaq(types) {

    // 問題排序

    const order = ['付款問題', '取貨問題', '訂單問題', '文章問題', '其他問題'];
    ...
    const questions = [
        ...
        },
        {
            ...
            answer: '因為我們必須經由人工核對的流程，如果您已經付款，需要等一些時間，並不表示您的付款過程有問題哦！<br>如果真的有些狀況，我們會主動聯繫您，沒有消息就是好消息！',
        },
    ];
    ...
    const featuredQuestions = computed(() => {
        ...
        if (types.length) {
            ...
            return sortQuestions.filter((q) => types.includes(q.type));

        }

        // 如果沒有特別指定分類，就從原始資料裡挑前六筆

        return questions.filter((q) => q.id < 7);

    });

    return { sortQuestions, featuredQuestions };
